...
function mixedBuilder() {
  ...
  return `<div class="mixed-builder"><div class="builder-title common-title"><strong>Common Drinks</strong><span>Tap once to add the complete recipe</span></div><div class="quick-builds">${Object.entries(quickMixedRecipes).map(([key,recipe])=>`<button data-quick-mixed="${key}"><strong>${esc(recipe.name)}</strong><span>${esc(label(recipe.spirit))} · ${esc(label(recipe.mixer))}</span><em>+ Add</em></button>`).join("")}</div><div class="builder-title"><strong>Build Your Own</strong><span>Choose an alcohol and what to mix it with</span></div><label>1 · Alcohol</label><div class="builder-scroll">${alcoholOptions.map(item=>`<button data-build-spirit="${esc(item)}" class="${b.spirit===item?"active":""}">${esc(label(item))}</button>`).join("")}</div><label>2 · Mix with</label><div class="builder-scroll">${mixerOptions.map(item=>`<button data-build-mixer="${esc(item)}" class="${b.mixer===item?"active":""}">${esc(label(item))}</button>`).join("")}</div>${b.spirit?.includes("Sparkling Wine")?"":`<label>3 · Pour</label><div class="builder-options"><button data-build-pour="1" class="${b.pour===1?"active":""}">Single</button><button data-build-pour="2" class="${b.pour===2?"active":""}">Double</button></div>`}<label>${b.spirit?.includes("Sparkling Wine")?"3":"4"} · Finish</label><div class="builder-scroll">${["Ice","No Ice","Lemon packet","Lime packet","Grapefruit packet"].map(mod=>`<button data-builder-modifier="${mod}" class="${b.modifiers.includes(mod)?"active":""}">${mod}</button>`).join("")}</div><button data-add-mixed="custom" class="add-build" ${b.spirit&&b.mixer?"":"disabled"}>Add mixed drink to ${state.seat}</button></div>`;
}
...
function prepareView() {
  ...
  return `<section class="workflow-panel"><div class="workflow-heading"><div><p class="eyebrow">Galley view</p><h2>Prepare orders</h2></div><button data-mode="take" class="secondary">Add orders</button></div>${foodSummary?`<div class="prep-food-summary"><p>First Class food tally</p>${foodSummary}</div>`:""}${orders.length?`<div class="prep-list">${orders.map(order=>`<article class="${order.delivered?"done":""}"><strong class="seat-chip">${order.seat}</strong><div class="seat-drinks">${(order.foods||[]).map(food=>{const item=state.foodMenu.find(x=>x.id===food.id);return `<div class="food-detail"><h3>${esc(item?.name||"Food item")}${food.qty>1?` ×${food.qty}`:""}</h3><p>Food</p></div>`}).join("")}${order.drinks.map(drink=>`<div><h3>${esc(drinkTitle(drink))}</h3><p>${esc(details(drink))}</p></div>`).join("")}</div><button data-edit="${order.seat}">Edit</button></article>`).join("")}</div>`:empty()}<button data-mode="deliver" class="primary wide" ${orders.length?"":"disabled"}>Ready to deliver</button></section>`;
}
function deliverView() {
  const orders=orderList();
  return `<section class="workflow-panel"><div class="workflow-heading"><div><p class="eyebrow">Cabin view</p><h2>Deliver orders</h2></div><button data-mode="prepare" class="secondary">Back</button></div>${orders.length?`<div class="delivery-grid">${orders.map(order=>{const total=order.drinks.reduce((n,drink)=>n+drink.qty,0)+(order.foods||[]).reduce((n,food)=>n+food.qty,0);return `<button data-deliver="${order.seat}" class="${order.delivered?"delivered":""}"><strong>${order.seat}</strong><div class="delivery-drinks">${(order.foods||[]).map(food=>{const item=state.foodMenu.find(x=>x.id===food.id);return `<div class="food-delivery"><b>${esc(item?.name||"Food item")}${food.qty>1?` ×${food.qty}`:""}</b><em>Food</em></div>`}).join("")}${order.drinks.map(drink=>`<div><b>${esc(drinkTitle(drink))}</b><em>${esc(details(drink))}</em></div>`).join("")}</div><small>${order.delivered?"Delivered ✓":`${total} item${total===1?"":"s"} · Tap when delivered`}</small></button>`}).join("")}</div>`:empty()}</section>`;
}
...
app.addEventListener("click",event=>{
  ...
  if(target.dataset.quickMixed){const recipe=quickMixedRecipes[target.dataset.quickMixed],order=state.orders[state.seat]||(state.orders[state.seat]={seat:state.seat,drinks:[],foods:[],delivered:false});const found=order.drinks.findIndex(item=>item.preset===target.dataset.quickMixed);if(found>=0){order.drinks[found].qty+=1;state.activeDrink=found}else if(recipe){order.drinks.push({drink:recipe.name,category:"Mixed Drinks",preset:target.dataset.quickMixed,spirit:recipe.spirit,mixer:recipe.mixer,pour:recipe.pour,modifiers:[...recipe.modifiers],creamer:null,creamQty:0,sweetenerType:null,sweetenerQty:0,qty:1});state.activeDrink=order.drinks.length-1}order.delivered=false}
  if(target.dataset.addMixed){const b=state.builder;if(b.spirit&&b.mixer){const order=state.orders[state.seat]||(state.orders[state.seat]={seat:state.seat,drinks:[],foods:[],delivered:false});const draft={drink:"Custom Mixed Drink",category:"Mixed Drinks",spirit:b.spirit,mixer:b.mixer,pour:b.pour,modifiers:[...b.modifiers],creamer:null,creamQty:0,sweetenerType:null,sweetenerQty:0,qty:1};draft.drink=mixedName(draft);order.drinks.push(draft);order.delivered=false;state.activeDrink=order.drinks.length-1;state.builder={spirit:null,mixer:null,pour:1,modifiers:["Ice"]}}}
  if(target.dataset.drink){const order=state.orders[state.seat]||(state.orders[state.seat]={seat:state.seat,drinks:[],foods:[],delivered:false});const found=order.drinks.findIndex(item=>item.drink===target.dataset.drink);if(found>=0){order.drinks[found].qty+=1;state.activeDrink=found}else{order.drinks.push({drink:target.dataset.drink,category:state.category,modifiers:[],creamer:null,creamQty:0,sweetenerType:null,sweetenerQty:0,qty:1});state.activeDrink=order.drinks.length-1}order.delivered=false}
  if(target.dataset.drinkDelta!==undefined){const order=currentOrder(),index=Number(target.dataset.index);order.drinks[index].qty+=Number(target.dataset.drinkDelta);if(order.drinks[index].qty<=0)order.drinks.splice(index,1);if(!order.drinks.length&&!(order.foods||[]).length)delete state.orders[state.seat];state.activeDrink=null}
