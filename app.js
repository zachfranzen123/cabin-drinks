...
const drinkCount = () => orderList().reduce((sum, order) => sum + order.drinks.length, 0);
...
function header() {
  const count = orderList().filter(order => !order.delivered).reduce((sum, order) => sum + order.drinks.length, 0);
...
function takeView() {
  ...
  const map=rows().map(row => `<div class="seat-row ${row===16?"exit-start":""}">${row===16?`<div class="exit-label">EXIT ROW${rows().includes(17)?"S":""}</div>`:""}<strong>${row}</strong>${seatLetters.map(letter => { const seat=`${row}${letter}`, item=state.orders[seat]; return `<button data-seat="${seat}" class="${state.seat===seat?"selected":""} ${item?"ordered":""}" aria-label="${seat}${item?`, ${item.drinks.length} drinks`:""}">${state.seat===seat?seat:letter}${item?`<span>${item.drinks.length}</span>`:""}</button>`; }).join("")}</div>`).join("");
  ...
  const selectedDrinks = order?.drinks.length ? `<div class="seat-order-list"><p>${order.drinks.length} drink${order.drinks.length===1?"":"s"} for ${state.seat}</p>${order.drinks.map((drink,index)=>`<button data-select-drink="${index}" class="${index===state.activeDrink?"active":""}"><span><strong>${esc(label(drink.drink))}</strong><small>${esc(details(drink))}</small></span><i data-remove-drink="${index}" aria-label="Remove ${esc(label(drink.drink))}">×</i></button>`).join("")}</div>` : "";
  ...
  return `<section class="seat-map" aria-label="Premium Class seat map"><div class="seat-head"><span></span><b>A</b><b>B</b><b>C</b><i></i><b>D</b><b>E</b><b>F</b></div>${map}</section><section class="order-panel"><div class="selected-line"><div><span>Selected seat</span><strong>${state.seat}</strong></div></div>${selectedDrinks}<div class="category-tabs">${Object.keys(menu).map(cat=>`<button data-category="${cat}" class="${state.category===cat?"active":""}">${cat}</button>`).join("")}</div><div class="drink-grid">${drinks}</div>${modifierEditor}</section><footer class="order-tray"><div><span>${order?.drinks.length?`${state.seat} · ${order.drinks.length} drink${order.drinks.length===1?"":"s"}`:`${state.seat} · Add a drink`}</span><small>Tap another drink to add it to this seat</small></div><button data-mode="prepare" ${drinkCount()?"":"disabled"}>Prepare · ${drinkCount()}</button></footer>`;
}

function prepareView() {
  const orders=orderList();
  return `<section class="workflow-panel"><div class="workflow-heading"><div><p class="eyebrow">Galley view</p><h2>Prepare drinks</h2></div><button data-mode="take" class="secondary">Add orders</button></div>${orders.length?`<div class="prep-list">${orders.map(order=>`<article class="${order.delivered?"done":""}"><strong class="seat-chip">${order.seat}</strong><div class="seat-drinks">${order.drinks.map(drink=>`<div><h3>${esc(drink.drink)}</h3><p>${esc(details(drink))}</p></div>`).join("")}</div><button data-edit="${order.seat}">Edit</button></article>`).join("")}</div>`:empty()}<button data-mode="deliver" class="primary wide" ${orders.length?"":"disabled"}>Ready to deliver</button></section>`;
}
function deliverView() {
  const orders=orderList();
  return `<section class="workflow-panel"><div class="workflow-heading"><div><p class="eyebrow">Cabin view</p><h2>Deliver drinks</h2></div><button data-mode="prepare" class="secondary">Back</button></div>${orders.length?`<div class="delivery-grid">${orders.map(order=>`<button data-deliver="${order.seat}" class="${order.delivered?"delivered":""}"><strong>${order.seat}</strong><span>${order.drinks.map(drink=>esc(label(drink.drink))).join(" + ")}</span><small>${order.delivered?"Delivered ✓":`${order.drinks.length} drink${order.drinks.length===1?"":"s"} · Tap when delivered`}</small></button>`).join("")}</div>`:empty()}<button data-action="clear" class="clear">Clear flight</button></section>`;
}
...
app.addEventListener("click",event=>{
  ...
  if(target.dataset.drink){const order=state.orders[state.seat]||(state.orders[state.seat]={seat:state.seat,drinks:[],delivered:false});order.drinks.push({drink:target.dataset.drink,category:state.category,modifiers:[],creamer:null,creamQty:0,sugarQty:0});order.delivered=false;state.activeDrink=order.drinks.length-1}
